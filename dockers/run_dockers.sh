...
docker run --gpus all -itd --shm-size=16g --ulimit memlock=-1  --ulimit core=0 --ulimit stack=67108864 --privileged --cap-add=IPC_LOCK --name "gpt_test2_sjf" --ipc host --net host -v "/data/sunjinfeng":"/data/sunjinfeng" "ngc-ssh/pytorch:21.07-py3" bash -c "sed -i 's/Port 62620/Port 11097/g' /root/.ssh/config && /usr/sbin/sshd -p 11097 && bash"
